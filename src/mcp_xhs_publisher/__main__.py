    ...
    try:
        ...
        if not in_mcp_mode:
            ...
            log_info(
                ...
                resources=["xhs-note://{note_id}: 笔记资源", "xhs-user: 用户资源"],
        ...
        log_error("服务器运行出错", error=str(e), error_type=type(e).__name__)
        raise
...
def main() -> None:
    ...
    try:
        ...
        # 记录日志但不输出到stderr
        log_error("服务器启动失败", error=str(e), error_type=type(e).__name__)
        sys.exit(1)
