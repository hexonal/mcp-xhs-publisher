...
def test_publish_image(publisher, tmp_path):
    ...
    result = publisher.publish_image("图文内容", image_paths=[str(img_path)], topics=["图文"])
    assert isinstance(result, dict)
    assert result["status"] in ("success", "error")
...
def test_publish_video(publisher, tmp_path):
    ...
    result = publisher.publish_video("视频内容", video_path=str(video_path), cover_path=None, topics=["视频"])
    assert isinstance(result, dict)
    assert result["status"] in ("success", "error")
